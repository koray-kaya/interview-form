...
vi.mock("@/db", () => ({
  ...
  deleteSmokeResponses: vi.fn(async () => 1),
...
describe("GET /api/cron/daily", () => {
  ...
  it("keeps the project awake and deletes what was left unfinished for more than seven days", async () => {
    ...
    expect(await response.json()).toEqual({ deleted: 2, smoke: 1, exported: 0 });
  ...
  it("writes a private export to Blob when a token is configured", async () => {
    ...
    vi.setSystemTime(new Date("2026-10-10T03:00:00Z"));
    const response = await run(`Bearer ${SECRET}`);
    expect(await response.json()).toEqual({ deleted: 2, smoke: 1, exported: 1 });
  ...
  it("removes the smoke test's responses, and cleans up before the export so a Blob failure keeps nothing", async () => {
    vi.stubEnv("BLOB_READ_WRITE_TOKEN", "vercel_blob_rw_token");
    vi.mocked(put).mockRejectedValueOnce(new Error("blob down"));
    await expect(run(`Bearer ${SECRET}`)).rejects.toThrow("blob down");
    expect(db.deleteSmokeResponses).toHaveBeenCalledOnce();
    expect(db.deleteUnfinishedBefore).toHaveBeenCalledOnce();
  });
});
